...
function Test(){
    ...
    useEffect(()=>{
        fetch('http://localhost:8080/item/orderBy/'+orderBy, {headers: {
                Authorization: localStorage.getItem("token")}})
            .then(response => response.json())
            .then(setList);
    }, [orderBy]);
            ...
            <Box height={50} width='100%' marginLeft={2}>
                <HStack spacing={2}>
                    ...
                    {orderBy === "byTitleReverse" ? <div>
                        <Button colorScheme={"blue"} width={"100px"} onClick={() => setOrderBy("byID")}>By title <Icon as={AiFillCaretDown} /></Button>
                        <Button width={"100px"} onClick={() => setOrderBy("byCost")}>By cost </Button>
                        <Button width={"140px"} onClick={() => setOrderBy("byPopularity")}>By popularity </Button>
                    </div> : null}
                    {orderBy === "byCost" ? <div>
                    ...
                    {orderBy === "byCostReverse" ? <div>
                        <Button width={"100px"} onClick={() => setOrderBy("byTitle")}>By title </Button>
                        <Button colorScheme={"blue"} width={"100px"} onClick={() => setOrderBy("byID")}>By cost <Icon as={AiFillCaretDown} /></Button>
                        <Button width={"140px"} onClick={() => setOrderBy("byPopularity")}>By popularity </Button>
                    </div> : null}
                    ...
                    {orderBy === "byPopularityReverse" ? <div>
                        <Button width={"100px"} onClick={() => setOrderBy("byTitle")}>By title </Button>
                        <Button width={"100px"} onClick={() => setOrderBy("byCost")}>By cost </Button>
                        <Button colorScheme={"blue"} width={"140px"} onClick={() => setOrderBy("byID")}>By popularity <Icon as={AiFillCaretDown} /></Button>
                    </div> : null}
