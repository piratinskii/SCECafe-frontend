...
function Cart() {
    ...
    return (
        <ChakraProvider>
            {inProgress.length > 0 && (localStorage.getItem('role') !== 'GUEST') ? (
                <Center marginTop={50}>
                    <VStack>
            ...
            {list.length > 0 ? (
                <div>
                            ...
                            <Card key={index} minW={500} maxW={500} borderRadius={10}>
                                <CardBody>
                                    <HStack justify="space-between">
                                        <Text fontSize='xl'>{index + 1}. {title} <Badge ml='1' fontSize='0.8em' w='30px'
                                                                                        h='30px' colorScheme='gray'
                                                                                        right='0px'
                                                                                        borderRadius='10px'>{cost}₪</Badge></Text>
                                        <NumberInput size='xs' maxW={16} defaultValue={count} min={1}
                                                     onChange={evt => handleChange(itemID, evt)}>
                                            <NumberInputField/>
                                            <NumberInputStepper>
                                                <NumberIncrementStepper/>
                                                <NumberDecrementStepper/>
                                        ...
                                        <Text fontSize='xl'>
                                            <Badge ml='1' fontSize='0.8em' w='30px' h='30px' colorScheme='gray'
                                                   right='0px' borderRadius='10px'>{sum}₪</Badge>
                                        </Text>
                                        <IconButton
                    ...
                    <Button colorScheme='messenger' fontSize='lg' marginTop='20px' right='0px'
                            onClick={() => toPayment()}>Let's buy</Button>
                </div>
            ) : (
                <Center>
                    <Heading marginTop={12} size={"lg"} minW={500} maxW={500}>
                        That's time to order coffee ;) Let`s see <Link href='http://localhost:3000/'
                                                                       color={"blue"}>menu</Link>
                    </Heading>
                </Center>
            )}
        </ChakraProvider>
